...
while True:
    bola.MoveBola()
    if bola.xcor() < -420:
        placar.P2();bola.Again()
    if bola.xcor() > 420:
        placar.P1();bola.Again()
    if bola.distance(player1) < 50 and bola.xcor() > 370:
        bola.xpogo = -bola.velocidade
    if  bola.distance(player2) < 50 and bola.xcor() < -370:
        bola.xpogo = bola.velocidade
    bola.Pong()
    if placar.p1 >= 10 or 10 <= placar.p2:
        break
